...
import { DefenseService } from 'src/app/services/defence.service';
...
constructor(private http: HttpClient,private  evaluationService: EvaluationService,    private defenceService: DefenseService,
...
/*private fetchChartData(): void {
  this.defenceService.getAllDefenses().subscribe({ // Added parentheses to call the method
      next: (data: defense[]) => {
          ...
          const groupedData = data.reduce((acc, item) => {
              const weekNumber = this.getWeekNumber(new Date(item.dateDefense)); // Assuming dateDefense is a string; adjust if necessary
              if (!acc[weekNumber]) {
                  acc[weekNumber] = [];
              }
          ...
          this.chartOptions.series = [{
              name: `Defenses Date (${weekNumber})`, // Use template literals correctly
              data: seriesData
          }];
