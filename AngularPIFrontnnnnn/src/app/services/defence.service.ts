...
export class DefenseService {
  ...
  private readonly GATEWAY_URL = 'http://localhost:8085/defence-service/api';

  constructor(private http: HttpClient) {}

  // Core Defense Operations
  getAllDefenses(): Observable<defense[]> {
    return this.http.get<defense[]>(`${this.GATEWAY_URL}/defences`)
      .pipe(
        retry(2),
  ...
  getDefenseById(defenseId: number): Observable<defense> {
    return this.http.get<defense>(`${this.GATEWAY_URL}/defences/${defenseId}`)
      .pipe(
        retry(2),
        catchError(this.handleError)
      );
  ...
  createDefense(defense: Omit<defense, 'idDef'>): Observable<defense> {
    return this.http.post<defense>(`${this.GATEWAY_URL}/defences`, defense)
      .pipe(
        catchError(this.handleError)
      );
  }
  ...
  deleteDefense(defenseId: number): Observable<void> {
    return this.http.delete<void>(`${this.GATEWAY_URL}/defences/${defenseId}`)
      .pipe(
        catchError(this.handleError)
      );
  }
  ...
  getUsedDates(): Observable<Date[]> {
    return this.http.get<Date[]>(`${this.GATEWAY_URL}/defences/used-dates`)
      .pipe(
        catchError(this.handleError)
      );
  }
